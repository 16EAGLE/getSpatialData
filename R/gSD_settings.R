...
#' \code{set_aoi} globally defines an AOI that is used for querying data within the running session (if no other AOI is provided with a query function call). The function supports \code{sf} and \code{sp} objects (see argument \code{aoi}) and is based on \code{sf}.
