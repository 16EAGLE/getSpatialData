#' Query Landsat data from the USGS Earth Explorer
#'
...
getLandsat_query <- function(time_range, name , aoi = NULL, username = NULL, password = NULL){
  ...
  query <- GET(paste0('https://earthexplorer.usgs.gov/inventory/json/v/1.4.0/search?jsonRequest={"apiKey":"', api.key,'","datasetName":"', name,'",',spatialFilter,',', temporalFilter, ',"startingNumber":1,"sortOrder":"ASC"}')) #"maxResults":100000000,
  ...
  if(length(query.results) != 0){
    ...
    return(do.call(rbind.data.frame, query.df))
