...
#'
#' \code{prepSentinel} makes downloaded Sentinel datasets ready-to-use by automatically extracting, sorting and converting the relevant contents of the datasets to a user-defined format.
#'
#' @inheritParams getSentinel_data
#' @param dir_zip character, path to zip file.
#' @param dir_out character, full path to target directory. Optional. If not set, \code{getSentinel_data} uses the directory to the \code{getSpatialData} archive folder. Use \link{set_archive} to once define a getSpatialData  archive folder.
...
prepSentinel <- function(dir_zip, dir_out = NULL, format = "vrt", select.tiles = "all", overwrite = F, verbose = T){
  ...
  char_args <- list(dir_zip = dir_zip, format = format)
  ...
  #look into archives
  out("Inspecting archive...")
  zip.files <- unzip(dir_zip, list = T)$Name
  ...
  if(select.tiles[1] != "all") tiles <- sapply(select.tiles, function(x, y = tiles) grep(x, y, value = T), USE.NAMES = F)
  ...
  dir.extr <- paste0(gsub(".zip", "", dir_zip), "/", granules)
  catch <- sapply(dir.extr, function(x) if(!dir.exists(x)) dir.create(x, recursive = T))
  files.jp2 <- mapply(x = f.jp2.tile, y = dir.extr, function(x, y, z = dir_zip){
    suppressWarnings(unzip(zipfile = dir_zip, exdir = y, files = x, overwrite = overwrite))
    paste0(y, "/", x)
  }, SIMPLIFY = F)

  #get MTD and level
  mtd.file <- grep("DATASTRIP", grep("GRANULE", grep("MTD", grep(".xml", zip.files, value = T), value = T), invert = T, value = T), invert = T, value = T)
  dir.mtd <- gsub(".zip", "", dir_zip)
  mtd.file <- suppressWarnings(unzip(zipfile = dir_zip, exdir = dir.mtd, files = mtd.file, overwrite = overwrite, junkpaths = T))
  ...
  #prep
  out("Checking 'vrt' files and stacking bands to 'vrt' files...")
  ...
  catch <- mapply(bjp2 = bands.jp2, v = vrt, function(bjp2, v, owr = overwrite) mapply(x = bjp2, y = v, FUN = function(x, y, ow = owr){
  ...
  catch <- mapply(bjp2 = bands.jp2, t = txt, function(bjp2, t, owr = overwrite) mapply(x = bjp2, y = t, FUN = function(x, y, ow = owr){
    mapply(x2 = x, y2 = y, FUN = function(x2, y2, ow2 = ow){
  ...
  if(format != "vrt"){
    out(paste0("Checking '", format, "' files and converting 'vrt' files to '", format, "' files..."))
    dst <- lapply(vrt, function(v, fo1 = format) lapply(v, function(x, fo2 = fo1) gsub(".vrt", paste0(".", fo2), x)))
    catch <- mapply(x = unlist(vrt), y = unlist(dst), function(x, y, ow = overwrite) if(!isTRUE(ow) & !file.exists(y)) gdal_translate(x, y), SIMPLIFY = F)
  } else{
    dst <- vrt
  }
