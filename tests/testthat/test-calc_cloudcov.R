...
error_test_calc_cloudcov <- function(records, aoi, tt) {
  ...
  # records type
  expect_error(calc_cloudcov(records = "Dumbledore"), dir_out = tt$tmp, RECORDS_TYPE_ERROR)
  ...
  # dir_out
  input1_dir_out <- 10
  error1_dir_out <- type_error_msg(input1_dir_out)
  input2_dir_out <- "Lord Voldemort"
  error2_dir_out <- dir_out_error_msg(input2_dir_out) # dir does not exist message
  expect_error(calc_cloudcov(records, aoi, dir_out = input1_dir_out), error1_dir_out)
  expect_error(calc_cloudcov(records, aoi, dir_out = input2_dir_out), error2_dir_out)
  # username
  input_username <- 11
  error_username <- type_error_msg(input_username, "password", CHARACTER)
  expect_error(calc_cloudcov(records, aoi, dir_out = tt$tmp, username = input_username), error_username)
  # password
  input_password <- 12
  error_password <- type_error_msg(input_password, "username", CHARACTER)
  expect_error(calc_cloudcov(records, aoi, dir_out = tt$tmp, password = input_password), error_password)
  # verbose
  input_verbose = "Avada Kedavra"
  error_verbose <- type_error_msg(input_verbose, "verbose", LOGICAL)
  expect_error(calc_cloudcov(records, aoi, dir_out = tt$tmp, verbose = input_verbose), error_verbose)
...
# with false input
error_test_calc_cloudcov(records, aoi_tunisia, tt)
...
records <- read.csv(construct_filepath(dir_records, SENTINEL2, SUFFIX$records))
records_cc1 <- expect_is(calc_cloudcov(records, aoi_tunisia, dir_out = tt$tmp), DATAFRAME)
...
records_cc2 <- expect_is(calc_cloudcov(records_previews, aoi_tunisia, dir_out = tt$tmp), DATAFRAME)
expect_length(names(records_cc1), length(records) + 5)
expect_length(names(records_cc2), length(records) + 3)
