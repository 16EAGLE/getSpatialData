if(isTRUE(auth)){
  ...
  prods <- vars$prods[vars$prods$group == "sentinel" | vars$prods$group == "landsat",]
  prods <- prods[!grepl("gnss", prods$product),]
  for(i in 1:nrow(prods)){
    msg <- paste0("get_records() for product group '", prods$group[i], "', product '", prods$product[i], "'")
    message(msg)
    test_that(msg,{
